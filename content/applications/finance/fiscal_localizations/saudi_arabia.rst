...
.. important::
   ...
   assign a tax on the line containing a global discount to comply with ZATCA rules.
...
.. note::
   Any negative tax (e.g., -20%) configured with the :guilabel:`Withhold On Payment` checkbox
   enabled has no impact on the UBL 2.1. It only applies when this retention tax is used in
   conjunction with a generic tax (e.g., 15%) on the invoice line and represents a percentage of
   that invoice line. For example, if a taxpayer wants to retain 20% of the invoiced goods, both the
   15% tax and the -20% retention tax would be applied to the invoice line.

.. important::
   When creating a new retention tax, create a :guilabel:`Retention` tax group and assign it to this
   newly created retention tax to comply with ZATCA rules.

Within this same tax position, retention can also be applied as an independent negative invoice line
using a 0% (Not Subject to VAT) tax.

.. warning::
   The 0% (Not Subject to VAT) tax is already preconfigured according to ZATCA rules; do not modify
   it.
...
.. note::
   PDF/A-3 with the embedded UBL 2.1 is generated automatically using either :guilabel:`Send` or
   :guilabel:`Print`.
...
- Onboard the :guilabel:`Invoices` default journal in the :ref:`PoS settings
  ...
  - Go to :menuselection:`Point of Sale --> Configuration --> Settings` and navigate to the
    :guilabel:`Accounting` section.
  - In the :guilabel:`Default Journals` section, click the :icon:`oi-arrow-right` :guilabel:`(right
    arrow)` in the :guilabel:`Invoices` field to open the default invoices journal.
  - Under the ZATCA tab, click :icon:`fa-refresh` :guilabel:`Re-Onboard`.
  - In the :guilabel:`Enter the OTP` window, enter the 6-digit :guilabel:`OTP` retrieved from the
