...
.. list-table::
   ...
   * - ODOO.BALANCE.TAG(account_tag_ids, [date_range], [offset], [company_id], [include_unposted])
     - Get the balance of a list of accounts (not compatible with Excel)
   ...
   * - ODOO.CURRENCY.RATE(currency_from, currency_to, [date])
     - This function takes in two currency codes as arguments, and returns the exchange rate from
       the first currency to the second as float (not compatible with Excel)
...
.. list-table::
   ...
   * - REGEXEXTRACT(text, pattern, [return_mode], [case_sensitivity])
     - `Excel EGEXEXTRACT article <https://support.microsoft.com/en-us/office/regexextract-function-4b96c140-9205-4b6e-9fbe-6aa9e783ff57>`_
   ...
   * - SPLIT(text, delimiter, [split_by_each], [remove_empty_text])
     - Divides text around a specified character or string, and puts each fragment into a separate
       cell in the row (not compatible with Excel)
